---
title: "Statistical Modeling with Missing Data"
output: rmarkdown::html_vignette
date: "`r Sys.Date()`"
vignette: >
  %\VignetteIndexEntry{Statistical Modeling with Missing Data}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---
  
This example demonstrates how to use the `cmest` function when there is missing data. For this purpose, we simulate some data containing a continuous baseline confounder $C_1$, a binary baseline confounder $C_2$, a binary exposure $A$, a binary mediator $M$ and a binary outcome $Y$. The true regression models for $A$, $M$ and $Y$ are:
...
To deal with missing data, we set the `multimp` argument to be `TRUE`. The regression-based approach is used for illustration. The multiple imputation results:
...
Compare the multiple imputation results with the results obtained from the dataset without missing data:
