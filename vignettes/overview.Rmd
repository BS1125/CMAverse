...
We categorize the approaches based on whether the approach deals with post-exposure confounding. *The regression-based approach*, *the weighting-based approach*, *the inverse odd-ratio weighting approach* and *the natural effect model* only deal with pre-exposure confounding. *The marginal structural model* and *the $g$-formula approach* deal with both pre-exposure confounding and post-exposure confounding.
...
Closed-form parameter function estimation is available when there is only a single mediator, i.e., $M=M_1$. Also, `yreg` must be chosen from `linear`, `logistic`, `loglinear`, `poisson`, `quasipoisson`, `negbin`, `coxph`, `aft_exp`, and `aft_weibull`. `mreg` must be chosen from `linear`, `logistic` and `multinomial`. The causal effects estimated through closed-form parameter function estimation are conditional on values of pre-exposure confounders specified by the `precval` argument. Closed-form parameter functions are summarized below.
