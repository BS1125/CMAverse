---
title: "Statistical Modeling with Post-exposure Confounding"
output: rmarkdown::html_vignette
date: "`r Sys.Date()`"
vignette: >
  %\VignetteIndexEntry{Statistical Modeling with Post-exposure Confounding}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---
  
This example demonstrates how to use the `cmest` function when there is post-exposure confounding. For this purpose, we simulate some data containing a continuous baseline confounder $C_1$, a binary baseline confounder $C_2$, a binary exposure $A$, a binary post-exposure confounder $L$, a binary mediator $M$ and a binary outcome $Y$. The true regression models for $A$, $L$, $M$ and $Y$ are:
