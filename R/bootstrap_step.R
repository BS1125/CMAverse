bootstrap_step <- function(data, indices, outcome, exposure, exposure.type,
                           mediator, covariates.pre,
                           covariates.post, covariates.post.type, vecc, model,
  ...
  regressions <- run_regressions(model = model, formulas = formulas, exposure.type = exposure.type,
  ...
  if (model == "rb") {
    ...
    if (EMint) { EMintTerm <- unname(thetas[stringr::str_replace_all(EMint.terms,
                                                                     pattern = "[*]", replacement = ":")])
