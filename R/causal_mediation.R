causal_mediation <- function(data, nway = c(3,4), type = c("delta", "bootstrap"), nboot = 100, conf = 0.95,
  ...
  bootstrap_3way_step <- function(data, indices, outcome, treatment, mediator, covariates, vecc,
    ...
    out <- c(cde = cde,
      ...
      out <- c(ORcde = cde,
               ORpnde = pnde, ORtnde = tnde,
               ORpnie = pnie, ORtnie = tnie,
               ORte = te,
               pm = pm)
...
delta_3way <- function(coef = NULL, vecc = c(), m_star = NULL, a_star = NULL, a = NULL) {
    ...
    if (j > 0) {
      for (i in 1:j) {
        cde_formula <- stringr::str_replace_all(cde_formula,
                                                paste("vecc", i, sep = "_"), as.character(vecc[i]))
      }
    }

    cde_formula <- as.formula(cde_formula)
    ...
    te_se_delta <- msm::deltamethod(te_formula, c(effect_estimates["ORpnde"], effect_estimates["ORtnie"]),
                                    Matrix::bdiag(pnde_se_delta^2, tnie_se_delta^2))

    pm_se_delta <- msm::deltamethod(pm_formula, c(effect_estimates["ORpnde"], effect_estimates["ORtnie"],
                                                  effect_estimates["ORte"]),
    ...
    if (yreg == "linear") {
      ...
      out <- c(cde = unname(effect_estimates["cde"]), cde_se_delta = cde_se_delta,
               pnde = unname(effect_estimates["pnde"]), pnde_se_delta = pnde_se_delta,
               tnde = unname(effect_estimates["tnde"]), tnde_se_delta = tnde_se_delta,
               pnie = unname(effect_estimates["pnie"]), pnie_se_delta = pnie_se_delta,
               tnie = unname(effect_estimates["tnie"]), tnie_se_delta = tnie_se_delta,
               te = unname(effect_estimates["te"]), te_se_delta = te_se_delta,
               pm = unname(effect_estimates["pm"]), pm_se_delta = pm_se_delta)

    } else if (yreg != "linear") {

      out <- c(ORcde = unname(effect_estimates["ORcde"]), ORcde_se_delta = cde_se_delta,
               ORpnde = unname(effect_estimates["ORpnde"]), ORpnde_se_delta = pnde_se_delta,
               ORtnde = unname(effect_estimates["ORtnde"]), ORtnde_se_delta = tnde_se_delta,
               ORpnie = unname(effect_estimates["ORpnie"]), ORpnie_se_delta = pnie_se_delta,
               ORtnie = unname(effect_estimates["ORtnie"]), ORtnie_se_delta = tnie_se_delta,
               ORte = unname(effect_estimates["ORte"]), ORte_se_delta = te_se_delta,
               pm = unname(effect_estimates["pm"]), pm_se_delta = pm_se_delta)
  ...
  delta_4way <- function(coef = NULL, vecc = c(), m_star = NULL, a_star = NULL, a = NULL) {
    ...
    if (yreg == "linear") {
      ...
      if (j > 0) {
        for (i in 1:j) {
          intref_formula <- stringr::str_replace_all(intref_formula,
                                                   paste("vecc", i, sep = "_"), as.character(vecc[i]))
        }
      }

      intref_formula <- as.formula(intref_formula)

      if (j > 0) {
        for (i in 1:j) {
          intmed_formula <- stringr::str_replace_all(intmed_formula,
                                                   paste("vecc", i, sep = "_"), as.character(vecc[i]))
        }
      }

      intmed_formula <- as.formula(intmed_formula)

      if (j > 0) {
        for (i in 1:j) {
          pie_formula <- stringr::str_replace_all(pie_formula,
                                                   paste("vecc", i, sep = "_"), as.character(vecc[i]))
        }
      }

      pie_formula <- as.formula(pie_formula)
      ...
      if (j > 0) {
        for (i in 1:j) {
          cde_comp_formula <- stringr::str_replace_all(cde_comp_formula,
                                                  paste("vecc", i, sep = "_"), as.character(vecc[i]))
        }
      }

      cde_comp_formula <- as.formula(cde_comp_formula)

      if (j > 0) {
        for (i in 1:j) {
          intref_comp_formula <- stringr::str_replace_all(intref_comp_formula,
                                                     paste("vecc", i, sep = "_"), as.character(vecc[i]))
        }
      }

      intref_comp_formula <- as.formula(intref_comp_formula)

      if (j > 0) {
        for (i in 1:j) {
          intmed_comp_formula <- stringr::str_replace_all(intmed_comp_formula,
                                                     paste("vecc", i, sep = "_"), as.character(vecc[i]))
        }
      }

      intmed_comp_formula <- as.formula(intmed_comp_formula)

      if (j > 0) {
        for (i in 1:j) {
          pie_comp_formula <- stringr::str_replace_all(pie_comp_formula,
                                                  paste("vecc", i, sep = "_"), as.character(vecc[i]))
        }
      }

      pie_comp_formula <- as.formula(pie_comp_formula)

      if (j > 0) {
        for (i in 1:j) {
          total_rr_formula <- stringr::str_replace_all(total_rr_formula,
                                                       paste("vecc", i, sep = "_"), as.character(vecc[i]))
        }
      }

      total_rr_formula <- as.formula(total_rr_formula)
  ...
  if (type == "delta") {
    ...
    if (nway == 3)
      effect_estimate <- format_df_delta(delta_3way(coef = coef, vecc = vecc, m_star = m_star,
                                               a_star = a_star, a = a),
    ...
    if (nway == 3)
      effect_estimate <- format_df_boot(bootstrap_3way(data = data, nboot = nboot,
                                                       outcome = outcome, treatment = treatment,
                                                       mediator = mediator, covariates = covariates,
                                                       vecc = vecc, interaction = interaction,
                                                       event = event, mreg = mreg, yreg = yreg,
                                                       m_star = m_star, a_star = a_star, a = a),
                                        conf = conf, n = n, nway = 3, yreg  = yreg)
    else if (nway == 4)
