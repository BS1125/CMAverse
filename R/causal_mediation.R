causal_mediation <- function(data = NULL, outcome = NULL, exposure = NULL, exposure.type = "binary",
  ...
  if (!(model %in% c("ne", "wb")) && length(mreg) < length(mediator)) {
  ...
  formulas <- create_formulas(outcome = outcome, exposure = exposure, mediator = mediator,
                              covariates = covariates, EMint = EMint, MMint = MMint, EMMint = EMMint,
                              EMint.terms = EMint.terms, MMint.terms = MMint.terms,
                              EMMint.terms = EMMint.terms, event = event, mreg = mreg, yreg = yreg)

  regressions <- run_regressions(formulas = formulas, model = model, mreg = mreg, yreg = yreg, data = data)
...
##########################################################################################################
  ...
  if (model == "standard") {
    ...
    } else if (is.numeric(data[,covariates]) == FALSE) {
      stop("The standard model only supports numeric covariates ")
    }
    ...
    effect_estimates <- bootstrap_step_singleM(data = data, indices = c(1:nrow(data)), outcome = outcome,
                                     exposure = exposure, exposure.type = exposure.type,
                                     mediator = mediator, covariates = covariates, model = model,
    ...
    if (yreg == "linear") {
      ...
      pnde_se <- msm::deltamethod(pnde_formula, c(thetas, betas[[1]]), vcov_block[[1]])

      tnde_se <- msm::deltamethod(tnde_formula, c(thetas, betas[[1]]), vcov_block[[1]])

      pnie_se <- msm::deltamethod(pnie_formula, c(thetas, betas[[1]]), vcov_block[[1]])

      tnie_se <- msm::deltamethod(tnie_formula, c(thetas, betas[[1]]), vcov_block[[1]])

      intref_se <- msm::deltamethod(intref_formula, c(thetas, betas[[1]]), vcov_block[[1]])

      intmed_se <- msm::deltamethod(intmed_formula, c(thetas, betas[[1]]), vcov_block[[1]])

      te_se <- msm::deltamethod(te_formula, c(thetas, betas[[1]]), vcov_block[[1]])

      pm_se <- msm::deltamethod(pm_formula, c(thetas, betas[[1]]), vcov_block[[1]])

      pie_se <- pnie_se

      cde_prop_se <- msm::deltamethod(cde_prop_formula, c(thetas, betas[[1]]), vcov_block[[1]])

      intref_prop_se <- msm::deltamethod(intref_prop_formula, c(thetas, betas[[1]]), vcov_block[[1]])

      intmed_prop_se <- msm::deltamethod(intmed_prop_formula, c(thetas, betas[[1]]), vcov_block[[1]])

      pie_prop_se <- msm::deltamethod(pie_prop_formula, c(thetas, betas[[1]]), vcov_block[[1]])

      overall_pm_se <- msm::deltamethod(overall_pm_formula, c(thetas, betas[[1]]), vcov_block[[1]])

      overall_int_se <- msm::deltamethod(overall_int_formula, c(thetas, betas[[1]]), vcov_block[[1]])

      overall_pe_se <- msm::deltamethod(overall_pe_formula, c(thetas, betas[[1]]), vcov_block[[1]])
      ...
      pnde_rr_se <- msm::deltamethod(pnde_rr_formula, c(thetas, betas[[1]]), vcov_block[[1]])

      tnde_rr_se <- msm::deltamethod(tnde_rr_formula, c(thetas, betas[[1]]), vcov_block[[1]])

      pnie_rr_se <- pie_err_se <- msm::deltamethod(pnie_rr_formula, c(thetas, betas[[1]]), vcov_block[[1]])

      tnie_rr_se <- msm::deltamethod(tnie_rr_formula, c(thetas, betas[[1]]), vcov_block[[1]])

      te_rr_se <- total_err_se <- msm::deltamethod(te_rr_formula, c(thetas, betas[[1]]), vcov_block[[1]])

      pm_se <- msm::deltamethod(pm_formula, c(thetas, betas[[1]]), vcov_block[[1]])

      cde_err_se <-  msm::deltamethod(cde_err_formula, c(thetas, betas[[1]]), vcov_block[[1]])

      intref_err_se <-  msm::deltamethod(intref_err_formula, c(thetas, betas[[1]]), vcov_block[[1]])

      intmed_err_se <-  msm::deltamethod(intmed_err_formula, c(thetas, betas[[1]]), vcov_block[[1]])

      cde_err_prop_se <- msm::deltamethod(cde_err_prop_formula, c(thetas, betas[[1]]), vcov_block[[1]])

      intmed_err_prop_se <- msm::deltamethod(intmed_err_prop_formula, c(thetas, betas[[1]]), vcov_block[[1]])

      intref_err_prop_se <- msm::deltamethod(intref_err_prop_formula, c(thetas, betas[[1]]), vcov_block[[1]])

      pie_err_prop_se <- msm::deltamethod(pie_err_prop_formula, c(thetas, betas[[1]]), vcov_block[[1]])
    ...
    for (j in 1:nsims) {
      ...
      regressions_sim <- run_regressions(formulas = formulas, model = model, mreg = mreg, yreg = yreg, data = data_sim)

      # counterfactual design matrices for the mediator model
      mdesign_a <- cbind(data.frame(exposure = c(rep(a,nrow(data)))),data[,covariates])

      mdesign_a_star <- cbind(data.frame(exposure = c(rep(a_star,nrow(data)))),data[,covariates])
      ...
      m_a <- do.call(cbind, lapply(1:length(mediator_regression),
                                   FUN = function(x) predict(mediator_regression[[x]], newdata = mdesign_a, type = "response")))

      m_a_star <- do.call(cbind, lapply(1:length(mediator_regression),
                                        FUN = function(x) predict(mediator_regression[[x]], newdata = mdesign_a_star, type = "response")))

      # counterfactual design matrices for the outcome model
      ydesign0m <- cbind(data.frame(exposure = c(rep(a_star, nrow(data))), mediator = t(m_star)[rep(1,nrow(data)),]), data[,covariates])

      ydesign1m <- cbind(data.frame(exposure = c(rep(a, nrow(data))), mediator = t(m_star)[rep(1,nrow(data)),]), data[,covariates])

      ydesign00 <- cbind(data.frame(exposure = c(rep(a_star, nrow(data))), mediator = m_a_star), data[,covariates])

      ydesign01 <- cbind(data.frame(exposure = c(rep(a_star, nrow(data))), mediator = m_a), data[,covariates])

      ydesign10 <- cbind(data.frame(exposure = c(rep(a, nrow(data))), mediator = m_a_star), data[,covariates])

      ydesign11 <- cbind(data.frame(exposure = c(rep(a, nrow(data))), mediator = m_a), data[,covariates])
      ...
      EY0m_sim <- c(EY0m_sim, sum(y0m)/nrow(data)) #E(Ya0m*)

      EY1m_sim <- c(EY1m_sim, sum(y1m)/nrow(data)) #E(Ya1m*)

      EY00_sim <- c(EY00_sim, sum(y00)/nrow(data)) #E(Ya0Ma0)

      EY10_sim <- c(EY10_sim, sum(y10)/nrow(data)) #E(Ya1Ma0)

      EY01_sim <- c(EY01_sim, sum(y01)/nrow(data)) #E(Ya0Ma1)

      EY11_sim <- c(EY11_sim, sum(y11)/nrow(data)) #E(Ya1Ma1)
...
##########################################################################################################
  ...
  if (model == "ne") {
    ...
    if(EMMint == TRUE && !is.null(EMMint.terms)) {
...
##########################################################################################################
  ...
  if (model == "wb") {
    ...
    if (length(m_star) != length(mediator)) {
      stop("Please specify am m value for each mediator")
    }
  ...
  if (model == "rb") {
    ...
    if (est.method == "bootstrap") {
      ...
      if ((is.null(covariates) & !is.null(vecc))) {
        ...
        stop("The regression-based model only supports numeric covariates ")
      }
      ...
      effect_estimates <- bootstrap_step_multipleM(data = data, indices = 1:nrow(data),
                                                 ...
                                                 EMint = EMint, MMint = FALSE, EMMint = FALSE,
                                                 EMint.terms = EMint.terms,
                                                 event = event, mreg = mreg, yreg = yreg,
                                                 m_star = m_star, a_star = a_star, a = a)
    ...
    } else if (est.method == "delta") {
        stop("The regression-based model doesn't support the delta method")
      }
