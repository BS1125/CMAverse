...
##########################################reorganize the dataset################################
  ...
  } else if (is.null(mval) == FALSE) {

    if (length(mval) != length(mediator)) {
  ...
  if (!is.null(covariates.post)) {
    ...
    for (i in 1:length(covariates.post)) {
      if (covariates.post.type[i] %in% c("binary", "categorical")) {
        data[,covariates.post.type[i]] <- as.factor(data[,covariates.post.type[i]])
        levels(data[,covariates.post.type[i]]) <- 0:(length(levels(data[,covariates.post.type[i]])) - 1)
      }
    }
  }
...
if (model != "ne") {
  ...
  if (model == "rb" && est.method == "paramfunc") {
    ...
    if (is.null(covariates.pre)) {
      ...
      if ((length(covariates.pre) != length(cval))) {

        stop("length(covariates.pre) != length(cval), values of pre-exposure covariates needed")

      }

      vecc <- c()

      covariates.pre.new <- c()
  ...
  effect_estimate <- est_step(data = data, indices = 1:n, outcome = outcome, exposure = exposure,
  ...
  out <- list(effect_estimate = effect_estimate, effect_se = effect_se)
    ...
    formulas <- create_formulas(outcome=outcome, exposure=exposure, mediator=mediator,
                             ...
                             event=event, mreg=mreg, yreg=yreg, model=model)
