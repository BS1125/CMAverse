causal_mediation <- function(data = NULL, outcome = NULL, event = NULL,
      ...
      if (is.null(covariates.pre)) {
        ...
        for (i in 1:length(covariates.pre)) {
          ...
          if (is.numeric(data[, covariates.pre[i]])) {
            ...
            } else  {vecc <- c(vecc, mean(data[, covariates.pre[i]]))}
            ...
            } else  {vecc <- c(vecc, unname(colMeans(model.matrix(as.formula(paste0("~", covariates.pre[i])), data=data)[,-1])))}
  ...
  effect_estimate <- est_step(data = data,
