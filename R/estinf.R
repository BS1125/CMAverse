estinf <- function() {
  ...
  for (reg_name in c("yreg", "ereg", "mreg", "wmreg", "postcreg")) {
    ...
    if (model %in% c("wb", "iorw", "msm", "ne") &&
        ("rcreg" %in% class(reg) | "simexreg" %in% class(reg))) stop(
          "Measurement error correction currently only supports model = 'rb' or 'gformula'")
    ...
    if (!is.null(reg)) {
      ...
      if (reg_name %in% c("yreg", "ereg")) {
        ...
        if (get(paste0("is_multinom_", reg_name))) {
          call.mid <- get(paste0("call_", reg_name))
          call.mid$model <- TRUE
          assign(reg_name, eval.parent(call.mid))
          rm(call.mid)
        }
        ...
        assign(reg_name, lapply(1:length(reg.mid), function(x)
          if (get(paste0("is_multinom_", reg_name))[x]) {
            call.mid <- get(paste0("call_", reg_name))[[x]]
            call.mid$model <- TRUE
            eval.parent(call.mid)
          } else reg[[x]]))
  ...
  if (model == "rb") {
    ...
    if (!multimp) {
      ...
      if (inference == "bootstrap") {
        # bootstrap results
        boots <- boot::boot(data = data, statistic = est.rb, R = nboot, outReg = FALSE, full = full)
        ...
        environment(deltamethod) <- environment()
        effect.se <- deltamethod(data = data, yreg = yreg, mreg = mreg)
        # critical value
        z0 <- qnorm(1 - (1 - 0.95) / 2)
      ...
      # imputed data sets
      data_imp <- mice::complete(do.call(mice::mice, args_mice), action = "all")
      m <- length(data_imp)
      ...
      if (inference == "bootstrap") {
        ...
        # bootstrap results
        boots <- boot::boot(data = data, statistic = boot.step, R = nboot)
        # bootstrap percentile CIs
        effect.ci.low <- sapply(1:n_effect, function(x) quantile(x = boots$t[, x], probs = (1 - 0.95)/2, na.rm=TRUE))
        effect.ci.high <- sapply(1:n_effect, function(x) quantile(x = boots$t[, x], probs = 1 - (1 - 0.95)/2, na.rm=TRUE))
        # bootstrap p-values
        ...
        se_imp <- do.call(rbind, lapply(1:m, function(x)
          deltamethod(data = data_imp[[x]], yreg = est_imp[[x]]$reg.output$yreg,
                      mreg = est_imp[[x]]$reg.output$mreg[[1]])))
        # pool the results by Rubin's rule
        var_within <- colMeans(se_imp ^ 2)
        var_between <- colSums((effect_imp_df - t(replicate(m, effect.pe)))^2)/(m - 1)
    ...
    if ((is_lm_yreg | is_glm_yreg) &&
      ...
      # effect names
      if (full) effect_name <- c("cde", "pnde", "tnde", "pnie", "tnie", "te", "pm",
                                 "intref", "intmed", "pie", "cde(prop)", "intref(prop)", "intmed(prop)", "pie(prop)",
                                 "pm(overall)", "int(overall)", "pe(overall)")
      ...
      if (inference == "delta") effect.se[1:6] <- sapply(1:6, function(x)
        msm::deltamethod(as.formula("~exp(x1)"), effect.pe[x], effect.se[x]^2))
      ...
      # effect names
      if (full) effect_name <- c("RRcde", "RRpnde", "RRtnde", "RRpnie", "RRtnie", "RRte", "pm",
                                 "ERRcde", "ERRintref", "ERRintmed", "ERRpie",
                                 "ERRcde(prop)", "ERRintref(prop)", "ERRintmed(prop)", "ERRpie(prop)",
                                 "pm(overall)", "int(overall)", "pe(overall)")
    ...
    if (!multimp) {
      ...
      # bootstrap results
      boots <- boot::boot(data = data, statistic = est.gformula, R = nboot, outReg = FALSE, full = full)
      # bootstrap percentile CIs
      effect.ci.low <- sapply(1:n_effect, function(x) quantile(x = boots$t[, x], probs = (1 - 0.95)/2, na.rm = TRUE))
      effect.ci.high <- sapply(1:n_effect, function(x) quantile(x = boots$t[, x], probs = 1 - (1 - 0.95)/2, na.rm = TRUE))
      # bootstrap p-values
    ...
    if ((is_lm_yreg | is_glm_yreg) &&
      ...
      if (length(postc) == 0 && full) effect_name <-
          c("cde", "pnde", "tnde", "pnie", "tnie", "te", "pm",
            "intref", "intmed", "pie", "cde(prop)", "intref(prop)", "intmed(prop)", "pie(prop)",
            "pm(overall)", "int(overall)", "pe(overall)")
      if (length(postc) == 0 && !full) effect_name <- c("cde", "pnde", "tnde", "pnie", "tnie", "te")
      if (length(postc) != 0 && full) effect_name <-
          c("cde", "rpnde", "rtnde", "rpnie", "rtnie", "te", "rpm",
            "rintref", "rintmed", "rpie", "cde(prop)", "rintref(prop)", "rintmed(prop)", "rpie(prop)",
            "rpm(overall)", "rint(overall)", "rpe(overall)")
      ...
      # effect names
      if (length(postc) == 0 && full) effect_name <-
        c("RRcde", "RRpnde", "RRtnde", "RRpnie", "RRtnie", "RRte", "pm",
          "ERRcde", "ERRintref", "ERRintmed", "ERRpie",
          "ERRcde(prop)", "ERRintref(prop)", "ERRintmed(prop)", "ERRpie(prop)",
          "pm(overall)", "int(overall)", "pe(overall)")
      if (length(postc) == 0 && !full) effect_name <- c("RRcde", "RRpnde", "RRtnde", "RRpnie", "RRtnie", "RRte")
      if (length(postc) != 0 && full) effect_name <-
        c("RRcde", "rRRpnde", "rRRtnde", "rRRpnie", "rRRtnie", "RRte", "rpm",
          "ERRcde", "rERRintref", "rERRintmed", "rERRpie",
          "ERRcde(prop)", "rERRintref(prop)", "rERRintmed(prop)", "rERRpie(prop)",
          "rpm(overall)", "rint(overall)", "rpe(overall)")
    ...
    if (!multimp) {
      ...
      # bootstrap results
      boots <- boot::boot(data = data, statistic = est.wb, R = nboot, outReg = FALSE, full = full)
      # bootstrap percentile CIs
      effect.ci.low <- sapply(1:n_effect, function(x) quantile(x = boots$t[, x], probs = (1 - 0.95)/2, na.rm = TRUE))
      effect.ci.high <- sapply(1:n_effect, function(x) quantile(x = boots$t[, x], probs = 1 - (1 - 0.95)/2, na.rm = TRUE))
      # bootstrap p-values
    ...
    if (!multimp) {
      ...
      # point estimates of causal effects
      est <- est.iorw(data = data, indices = NULL, outReg = TRUE)
      effect.pe <- est$est
      n_effect <- length(effect.pe)
      out$reg.output <- est$reg.output

      # bootstrap results
      boots <- boot::boot(data = data, statistic = est.iorw, R = nboot, outReg = FALSE)
      # bootstrap percentile CIs
      effect.ci.low <- sapply(1:n_effect, function(x) quantile(x = boots$t[, x], probs = (1 - 0.95)/2, na.rm = TRUE))
      effect.ci.high <- sapply(1:n_effect, function(x) quantile(x = boots$t[, x], probs = 1 - (1 - 0.95)/2, na.rm = TRUE))
      # bootstrap p-values
      ...
      # estimate causal effects for each imputed data set
      est_imp <- lapply(1:m, function(x)
        est.iorw(data = data_imp[[x]], indices = NULL, outReg = TRUE))
      est_imp_df <- do.call(rbind, lapply(1:m, function(x) est_imp[[x]]$est))
      effect.pe <- colMeans(est_imp_df)
      n_effect <- length(effect.pe)
      out$reg.output <- lapply(1:m, function(x) est_imp[[x]]$reg.output)
      ...
      boot.step <- function(data = NULL, indices = NULL) {
        ...
        setTxtProgressBar(get("progbar", envir = env), curVal + 1)
        return(colMeans(do.call(rbind, lapply(1:m, function(x)
          est.iorw(data = data_imp[[x]], outReg = FALSE)))))
      }
      environment(boot.step) <- environment()
      # bootstrap results
      boots <- boot::boot(data = data, statistic = boot.step, R = nboot)
    ...
    if ((is_lm_yreg | is_glm_yreg) &&
      ...
      # effect names
      effect_name <- c("RRtot", "RRdir", "RRind")
    }
    ...
    if (!multimp) {
      ...
      # bootstrap results
      boots <- boot::boot(data = data, statistic = est.msm, R = nboot, outReg = FALSE, full = full)
      # bootstrap percentile CIs
      effect.ci.low <- sapply(1:n_effect, function(x) quantile(x = boots$t[, x], probs = (1 - 0.95)/2, na.rm = TRUE))
      effect.ci.high <- sapply(1:n_effect, function(x) quantile(x = boots$t[, x], probs = 1 - (1 - 0.95)/2, na.rm = TRUE))
      # bootstrap p-values
    ...
    if (!multimp) {
      ...
      # bootstrap results
      boots <- boot::boot(data = data, statistic = est.ne, R = nboot, outReg = FALSE, full = full)
      # bootstrap percentile CIs
      effect.ci.low <- sapply(1:n_effect, function(x) quantile(x = boots$t[, x], probs = (1 - 0.95)/2, na.rm = TRUE))
      effect.ci.high <- sapply(1:n_effect, function(x) quantile(x = boots$t[, x], probs = 1 - (1 - 0.95)/2, na.rm = TRUE))
      # bootstrap p-values
