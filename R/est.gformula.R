est.gformula <- function(data = NULL, indices = NULL, outReg = FALSE, full = TRUE) {
  ...
  if (casecontrol && !is.null(yprevalence)) {
    ...
    if (length(postc) != 0) {
      for (p in 1:length(postc)) {
        # weights for postcreg[[p]]
        if (!is.null(weights_postcreg[[p]])) weights_postcreg <- weights_postcreg[[p]][indices] * w4casecon
        if (is.null(weights_postcreg[[p]])) weights_postcreg <- w4casecon
        # update postcreg[[p]]
        call_postcreg[[p]]$weights <- weights_postcreg
  ...
  if (length(postc) != 0) {
    ...
    for (p in 1:length(postc)) {
      ...
      if ((is_glm_postcreg[p] && ((family_postcreg[[p]]$family %in% c("binomial", "multinom")) |
        ...
        if (dim(prob_a)[2] == 1) {
          # simulate postc[p] for exposure=a
          lsim_a <- rbinom(n, size = 1, prob = prob_a[, 1]) + 1
          # simulate postc[p] for exposure=astar
        ...
        l_lev <- levels(droplevels(as.factor(model.frame(postcreg[[p]])[, postc[p]])))
        ...
        } else if (is.logical(data[, postc[p]])) {
          mid_a <- as.logical(l_lev[lsim_a])
          mid_astar <- as.logical(l_lev[lsim_astar])
        } else stop("The postc[", p, "] variable should be numeric, logical, factor or character")
        ...
        rm(error)
        # gamma L
      } else if ((is_lm_postcreg[p] | is_glm_postcreg[p]) && family_postcreg[[p]]$family == "Gamma") {
        ...
        rm(lambda)
        # poisson L
      } else if ((is_lm_postcreg[p] | is_glm_postcreg[p]) && family_postcreg[[p]]$family == "poisson") {
        mid_a <- rpois(n, lambda = postcpred_a)
        mid_astar <- rpois(n, lambda = postcpred_astar)
        # quasipoisson L
      } else if ((is_lm_postcreg[p] | is_glm_postcreg[p]) && family_postcreg[[p]]$family == "quasipoisson") {
        ...
        rm(phi)
        # negative binomial L
      } else if ((is_lm_postcreg[p] | is_glm_postcreg[p]) && startsWith(family_postcreg[[p]]$family, "Negative Binomial")) {
  ...
  for (p in 1:length(mediator)) {
    ...
    # randomly shuffle values of simulated mediator[p]
    if (length(postc) != 0) {
